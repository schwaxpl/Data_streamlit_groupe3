...
if("model" in st.session_state):
    ...
    bt_predict = st.button("Traiter sur le fichier brut")
    ...
    if "data" in st.session_state:
        bt_predict2 = st.button("Traiter sur le dataset en mémoire")
        if bt_predict2:
            data_target = st.session_state["data"]
            if("target" in data_target.columns):
                data_target.drop("target",axis=1,inplace=True)
            data_target["prediction"] = model.predict(data_target)
            st.success("Votre Dataset a été correctement évalué. Voici un aperçu des données :",icon="✅" )
            st.dataframe(data_target.head())
            st.session_state["data_predict"] = data_target
